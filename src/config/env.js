...
console.log(
  `Loading environment variables from .env.${requestedEnvironment} and .env`,
);
  ...
  .object({
    ...
    FRONTEND_ORIGIN: z
      ...
      .refine(
        ...
        "FRONTEND_ORIGIN must contain one or more comma-separated URLs",
      )
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.JWT_ACCESS_SECRET.includes("replace-with"))
      ctx.addIssue({
        ...
        message: "Use a real secret outside development",
      });
    if (value.TOKEN_ENCRYPTION_KEY === "0123456789abcdef".repeat(4))
      ctx.addIssue({
        code: "custom",
        path: ["TOKEN_ENCRYPTION_KEY"],
        message: "Use a real encryption key outside development",
      });
    if (!value.COOKIE_SECURE)
      ctx.addIssue({
        code: "custom",
        path: ["COOKIE_SECURE"],
        message: "Secure cookies are required outside development",
      });
  });
