...
export class ReportingPeriodService{
  async list(user,teamPublicId){const team=await getTeamByPublicId(teamPublicId);await assertTeamMember(user,team.id);return reportingPeriodRepository.listByTeam(team.id);}
  async create(user,data){const team=await getTeamByPublicId(data.teamPublicId);await assertManagerTeamAccess(user,team.id);
