...
export class RoleRepository {
  ...
  async findByCode(code, db = this.db) {
    const [rows] = await db.execute(
      "SELECT id,code,name FROM m_roles WHERE code=? LIMIT 1",
      [code],
    );
    return rows[0] ?? null;
  }
  async list(db = this.db) {
    const [rows] = await db.execute(
      "SELECT id,code,name FROM m_roles ORDER BY id",
    );
    return rows;
  }
  async getUserRoleCodes(userId, db = this.db) {
    const [rows] = await db.execute(
      `SELECT r.code FROM user_roles ur JOIN m_roles r ON r.id=ur.role_id WHERE ur.user_id=?`,
      [userId],
    );
  ...
  async replaceUserRoles(userId, roleIds, assignedBy, db = this.db) {
